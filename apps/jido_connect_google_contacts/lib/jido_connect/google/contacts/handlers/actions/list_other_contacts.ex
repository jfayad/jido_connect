defmodule Jido.Connect.Google.Contacts.Handlers.Actions.ListOtherContacts do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    ...
    with {:ok, client} <- Helpers.fetch_client(credentials),
         {:ok, result} <- client.list_other_contacts(input, Map.get(credentials, :access_token)) do
