defmodule Jido.Connect.Gmail.Handlers.Actions.ListThreads do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- fetch_client(credentials),
         {:ok, result} <-
           client.list_threads(normalize_input(input), Map.get(credentials, :access_token)) do
      {:ok,
       %{
         threads: Enum.map(Map.get(result, :threads, []), &public_map/1),
