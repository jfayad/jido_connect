defmodule Jido.Connect.Google.Calendar.Handlers.Actions.ChannelLifecycle do
  @moduledoc false
  ...
  def validate_watch_input(input, required_fields) do
    with :ok <- validate_channel_id(input),
         ...
         :ok <- validate_channel_type(Data.get(input, :channel_type, "web_hook")),
         :ok <- validate_ttl(Data.get(input, :ttl_seconds)),
         :ok <- validate_expiration(Data.get(input, :expiration_ms)) do
  ...
  defp validate_channel_id(input) do
    with :ok <- require_present(input, :channel_id) do
      ...
      if String.length(channel_id) <= 64 do
        ...
        {:error,
         Error.validation("Google Calendar channel channel_id must be 64 characters or fewer",
  ...
  defp validate_address(input) do
    with :ok <- require_present(input, :address) do
      ...
      if uri.scheme == "https" and is_binary(uri.host) and uri.host != "" do
        ...
        {:error,
         Error.validation("Google Calendar channel address must be an HTTPS URL",
           reason: :invalid_calendar_channel,
  ...
  defp validate_ttl(ttl) do
    validation_error("Google Calendar watch ttl_seconds must be a positive integer",
      field: :ttl_seconds,
      value: ttl
    )
  end
  ...
  defp validate_expiration(expiration) when is_integer(expiration) and expiration > 0, do: :ok

  defp validate_expiration(expiration) do
    validation_error("Google Calendar watch expiration_ms must be a positive integer",
      field: :expiration_ms,
      value: expiration
    )
  end
  ...
  defp invalid_channel_type(type) do
    validation_error("Google Calendar watch channel_type is invalid",
      field: :channel_type,
      value: type,
  ...
  defp invalid_field(field) do
    validation_error("Google Calendar channel #{field} must be a non-empty string", field: field)
  end
