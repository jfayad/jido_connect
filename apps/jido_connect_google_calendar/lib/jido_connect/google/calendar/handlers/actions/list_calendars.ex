defmodule Jido.Connect.Google.Calendar.Handlers.Actions.ListCalendars do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- fetch_client(credentials),
         {:ok, result} <-
           client.list_calendars(normalize_input(input), Map.get(credentials, :access_token)) do
      {:ok,
       %{
         calendars: Enum.map(Map.get(result, :calendars, []), &public_map/1),
