defmodule Jido.Connect.Slack.Client.Rest do
  @moduledoc """
  Shared Slack Web API implementation used by the API-area client modules.

  Prefer calling `Jido.Connect.Slack.Client` or a narrower module such as
  `Jido.Connect.Slack.Client.Conversations`. This module centralizes shared
  transport and normalization helpers while the public surface is organized by
  API area.
  """
  ...
  def remove_pin(attrs, access_token) when is_map(attrs) and is_binary(access_token) do
    access_token
    |> request()
    |> Req.post(url: "/pins.remove", json: pin_params(attrs))
    |> handle_remove_pin_response(attrs)
  end
  ...
  defp handle_remove_pin_response({:ok, %{status: status, body: %{"ok" => true}}}, attrs)
       when status in 200..299 do
    {:ok,
     %{
       type: "message",
