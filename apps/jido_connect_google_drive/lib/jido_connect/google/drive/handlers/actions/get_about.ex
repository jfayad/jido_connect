defmodule Jido.Connect.Google.Drive.Handlers.Actions.GetAbout do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- fetch_client(credentials),
         {:ok, about} <- client.get_about(input, Map.get(credentials, :access_token)) do
      {:ok, %{about: public_map(about)}}
    end
  end
