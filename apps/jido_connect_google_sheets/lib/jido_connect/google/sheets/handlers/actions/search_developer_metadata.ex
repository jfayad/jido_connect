defmodule Jido.Connect.Google.Sheets.Handlers.Actions.SearchDeveloperMetadata do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with :ok <- validate_data_filters(input),
         ...
         {:ok, result} <-
           client.search_developer_metadata(input, Map.get(credentials, :access_token)) do
