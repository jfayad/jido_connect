defmodule Jido.Connect.Google.Sheets.Handlers.Actions.BatchGetValuesByDataFilter do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with :ok <- validate_data_filters(input),
         ...
         {:ok, result} <-
           client.batch_get_values_by_data_filter(input, Map.get(credentials, :access_token)) do
